    ...
    <div class="banner">
        <div class="slider-wrap">
            <ul class="slider-items" :style="[sliderActive,{'width':sliderImg.length*100+'vw'}]" @touchstart="stopSlider($event)" @touchmove="moveSlider($event)" @touchend="continSilder($event)">
                <li class="slider-item" v-for="(item,index) in sliderImg" @click.stop="linkURl(item)" :key="index"><img :src="getURl(item,index)" class="item-img"></li>
            </ul>
        </div>
        <ul class="banner-page">
            <li class="slider-btn" v-for="(item,index) in bannerlist" :class="{'active':index === nowSlider}" :key="index"></li>
        </ul>
    </div>
</template>
<script>
import reloadImg from '../assets/images/loading.gif'
import logoImg from '../assets/vux_logo.png'
